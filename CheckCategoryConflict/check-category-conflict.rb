...
#   该脚本用于检测 Category 冲突
#
#   Object-C 的 Category 初衷是为了将类分门别类的，根据功能、目的等进行分类，实现清晰整洁的编码风格。
#   同时，该技术也能对现有的类进行扩展，扩展出方便快捷的函数。
#   然而过度的不正确的使用，容易导致项目中存在 Category 同名函数冲突。
#   这种同名冲突无法自动检测，导致同名函数互相覆盖，代码最终执行到哪个具体函数不可确定。
#   直接后果是出现不可预期的、很难排查的问题。
#
...
def parse_category(filepath, prefix=nil)
	IO.popen("nm -arch #{ENV['CURRENT_ARCH']} \"#{filepath}\" 2> /dev/null | grep -oh ' [-+]\\[.*(.*) .*\\]$' ") do |io|
		io.read.split("\n").each do |line|
            # 排除 Category 的 load 类方法
            unless line.end_with?(" load]")
